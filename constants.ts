...
export const UPGRADES: { [id: string]: UpgradeData } = {
  base_hp: {
    ...
    description: level => `Tăng HP tối đa của Môn Phái thêm ${250}. Hiện tại: +${250 * level}`,
  },
  mana_regen: {
    id: 'mana_regen', name: 'Hồi Mana', icon: BoltIcon, maxLevel: 5,
    cost: level => 150 * (level + 1),
    description: level => `Tăng tốc độ hồi Mana thêm ${0.5}/s. Hiện tại: +${(0.5 * level).toFixed(1)}/s`,
  },
  unit_hp: {
    id: 'unit_hp', name: 'Lính HP', icon: HeartIcon, maxLevel: 5,
    cost: level => 200 * (level + 1),
    description: level => `Tăng HP của lính thêm ${10}%. Hiện tại: +${10 * level}%`,
  },
  unit_attack: {
    id: 'unit_attack', name: 'Lính Tấn Công', icon: SparklesIcon, maxLevel: 5,
    cost: level => 200 * (level + 1),
    description: level => `Tăng tấn công của lính thêm ${10}%. Hiện tại: +${10 * level}%`,
  }
};
